        ...
        if (message && message.body) {
            ...
            if (message.body === '/stormyStop') {
                ...
                var options = {
                    ...
                    json: {"longUrl": message.body.replace('/shorturl ', '')}
                }

                request(options, function (error, response, body) {
                    if (!error && response.statusCode == 200) {
                        api.sendMessage(body.longUrl + ' -> ' + body.id, message.threadID);
                        console.log(body.longUrl + ' -> ' + body.id);
                    }
                })
            } else if (message.body.includes('/search')) {
                https.get('https://www.googleapis.com/customsearch/v1?q=' + encodeURIComponent(message.body.replace('/search ', '')) + '&cx=' + process.env.ID_SEARCH + '&num=3&key=' + process.env.API_SEARCH, function (res) {
                    var body = '';
